#include "Piston_tests.hpp"

#include "../src/godunov_solver.hpp"
#include "Piston_functions.hpp"
...
void step_dependency() {
  ...
  for (N = 100; N <= 1000; N *= 10) {
    start_time = clock();
    dir_name =
        solver::solve_system(x_0, x_n, x_c, time, N, r_l, u_l, p_l, r_r, u_r, p_r, amplitude, omega, P_function,
...
void pressure_dependency() {
  ...
  start_time = clock();
  dir_name   = solver::solve_system(x_0, x_n, x_c, time, N, r_l, u_l, p_l, r_r, u_r, p_r, amplitude, omega, P_function,
                                    boundaries::piston, boundaries::piston, false);
  time_log << dir_name << ": \t" << (clock() - start_time) / CLOCKS_PER_SEC << std::endl;
